...
const UnderTo2000 = () => {
    ...
    const Under2000Product = AllProducts.filter(item => item.price > 1000 &&item.price <=2000)
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 m-5">
            {
                Under2000Product?.map(item=><ProductsCard key={item._id} productsDetails={item}></ProductsCard>)
            }
        </div>
    )
};

export default UnderTo2000;