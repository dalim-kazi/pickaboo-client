...
import MensSubCategory from "../../../../component/SubCategory/MensSubCategory";

 
const Mans = () => {
    const [AllProducts, isLoading] = UseProducts()
    const menProducts = AllProducts.filter(item => item.gender === "men")
    if (isLoading) {
    ...
    return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 m-5">
            {
                menProducts?.map(item=><ProductsCard key={item._id} productsDetails={item}></ProductsCard>)
            }
        </div>
        </>
    );
