...
const Boys = () => {
    ...
    return (
        <>
         <p className="text-2xl mt-20 mx-10 text-orange-600 border-b-2 text-center">Boys Collection</p>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 m-5">
            {
                boysProducts?.map(item=><ProductsCard key={item._id} productsDetails={item}></ProductsCard>)
            } 
        </div>
        </>
    );
